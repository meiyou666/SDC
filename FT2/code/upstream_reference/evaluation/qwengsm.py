...
def clamp_to_zero(tensor, lower, upper):
    ...
    valid_mean = torch.mean(valid_values)

    return torch.where(mask, valid_mean * torch.ones_like(tensor), tensor)
...
def normalize_answer(s):
    ...
    return white_space_fix(remove_articles(remove_latex(lower(s))))
...
if __name__ == '__main__':
    ...
    qid = read_qids_from_file("qidgsmfinal.txt")
    # print(qid[0])
    dataset = datasets.load_dataset("openai/gsm8k", "main", split="test")
    dataset1 = datasets.load_dataset("openai/gsm8k", "main", split="train")
    ...
    model = Qwen2ForCausalLM.from_pretrained("Qwen/Qwen2-Math-7B", torch_dtype=torch.float16, device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("gsmrandomtokenqwen.txt", "r") as f:
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for _ in range(20):
            ...
            for i in range(28):
                ...
                #handle15 = model.model.layers[i].mlp.down_proj.register_forward_hook(hook3c)
                #handles.append(handle2)
                #handles.append(handle3)
                #handles.append(handle4)
                #handles.append(handle5)
            ...
            answer2 = [ans.split('#### ')[-1].strip() for ans in answer2]
            for modify_output in generatedtext:
                ...
                #print("-------------------------------")
            # total_f1 += f11
                pre2, re2, f12 = compute_precision_recall_f1(modify_output, answer2[i])
            # total_pre2 += pre2
                if re2 < 1:
                    re2 == 0
                total_re2 += re2
