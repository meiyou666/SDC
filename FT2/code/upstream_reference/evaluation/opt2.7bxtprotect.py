...
if __name__ == '__main__':
    ...
    qid = read_qids_from_file("qidxtremefinal.txt")
    # print(qid[0])
    dataset = datasets.load_dataset("google/xtreme", "MLQA.en.en", split="validation")
    dataset1 = datasets.load_dataset("google/xtreme", "MLQA.en.en", split="test")
    ...
    model = OPTForFICausalLM.from_pretrained("facebook/opt-2.7b", torch_dtype=torch.float16, device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("xtremerandomtokenopt2.7b.txt", "r") as f:
        key_tokens = [int(line.strip()) for line in f]
    #print(key_tokens)

    global min1
    min1 = torch.arange(192, dtype=torch.float16).reshape(6, 32).fill_(1000)
    global max1
    max1 = torch.arange(192, dtype=torch.float16).reshape(6, 32).fill_(-1000)
    min1.to(device)
    max1.to(device)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for _ in range(20):
            ...
            if selected_layer == 'v_proj':
                ...
                handle1 = model.model.decoder.layers[b].self_attn.k_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'q_proj':
                handle1 = model.model.decoder.layers[b].self_attn.q_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'out_proj':
                handle1 = model.model.decoder.layers[b].self_attn.out_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'fc1':
                handle1 = model.model.decoder.layers[b].fc1.register_forward_hook(hook2_fn)
            else:  # fc2
                handle1 = model.model.decoder.layers[b].fc2.register_forward_hook(hook2_fn)
            #handle1 = model.model.decoder.layers[b].self_attn.v_proj.register_forward_hook(hook1_fn)
            #print(b)
            #handle1.remove()
