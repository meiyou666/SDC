#!/usr/bin/env python3
"""Summarize IV-A bit-13 per-kernel forward/backward sweep runs."""
...
def main():
    ...
    for manifest_row in load_manifest(args.manifest):
        ...
        safe_label = "".join(char if char.isalnum() or char == "_" else "_" for char in kernel_label)
        run_dir = args.checkpoint_root / safe_label
        ...
        rows.append({
            ...
            "profile_launch_count": manifest_row["launch_count"],
    ...
    print("\nIV-A bit-13 kernel sweep summary")
    print("kernel location status    eval_loss       delta           param_diff      triggers schedule nonfinite")
    for row in rows:
        print(
            f'{row["kernel_label"]:<6} {row["location"]:<8} {row["status"]:<9} '
    ...
    print(f"\nSaved IV-A kernel sweep summary to: {args.output}")
