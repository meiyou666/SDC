#!/usr/bin/env python3
"""Summarize one part (lane or sm) of the reduced Section IV-E spatial sweep."""
...
def value_of(run_dir, part):
    name = run_dir.name
    prefix = "lane_" if part == "lane" else "sm_"
...
def finite_or_blank(values):
    return [v for v in values if isinstance(v, float) and math.isfinite(v)]


def summarize(values):
    numbers = finite_or_blank(values)
    if not numbers:
        return {"n": 0, "mean": "", "pstdev": "", "min": "", "max": ""}
    return {
        "n": len(numbers),
        "mean": statistics.fmean(numbers),
        "pstdev": statistics.pstdev(numbers) if len(numbers) > 1 else 0.0,
        "min": min(numbers),
        "max": max(numbers),
    }
...
def main():
    ...
    parser.add_argument("--part", choices=["lane", "sm"], required=True)
    ...
    if not args.checkpoint_root.is_dir():
        raise ValueError(f"Checkpoint root not found: {args.checkpoint_root}")
    ...
    run_dirs = sorted(
        (d for d in args.checkpoint_root.iterdir() if d.is_dir() and value_of(d, args.part) is not None),
        key=lambda d: value_of(d, args.part),
    )
    if not run_dirs:
        raise ValueError(f"No {args.part} run directories under {args.checkpoint_root}")

    varied_key = "target_laneid" if args.part == "lane" else "target_smid"
    rows = []
    for run_dir in run_dirs:
        ...
        rows.append({
            varied_key: value_of(run_dir, args.part),
            "status": status,
    ...
    eval_stats = summarize([row["eval_loss_delta"] for row in complete_rows])
    param_stats = summarize([row["final_parameter_difference"] for row in complete_rows])

    print(f"\nIV-E {args.part} sweep summary ({len(rows)} runs, {len(complete_rows)} complete)")
    print(f"{varied_key:<13} status    eval_loss       delta           param_diff      triggers schedule nonfinite")
    for row in rows:
        print(
            f'{row[varied_key]:<13} {row["status"]:<9} '
    ...
    print(f"\nSpatial spread over complete runs (n={eval_stats['n']}):")
    print(f"  eval_loss_delta:            mean={eval_stats['mean']!s:<18} pstdev={eval_stats['pstdev']!s:<18} min={eval_stats['min']!s:<18} max={eval_stats['max']!s}")
    print(f"  parameter_difference (L2):  mean={param_stats['mean']!s:<18} pstdev={param_stats['pstdev']!s:<18} min={param_stats['min']!s:<18} max={param_stats['max']!s}")
    print(f"\nSaved IV-E {args.part} summary to: {args.output}")
