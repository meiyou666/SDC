...
def main(args):
    ...
    if args.base_model_continue:
        ...
        if os.path.exists(training_state_path):
            ...
            if "window_mean" in _old_state:
                restored_window_mean = float(_old_state["window_mean"])
    ...
    if restored_window_mean is not None:
        window_mean = torch.tensor(restored_window_mean, device=device)
        logger.info(f"Restored detection window_mean: {restored_window_mean:.6e}")
    else:
    ...
    for batch_idx, batch in enumerate(dataloader, start=start_batch_idx):
        ...
        # Schedule NVBit before launching this update. Previously an explicit
        # --fi_nvbit_steps value was checked after the optimizer step, causing
        # the actual injection and metrics marker to appear two steps later.
        ...
        if global_rank == 0:
            ...
            metrics_logger.log_dict(global_step, rt_dict)
    ...
    if global_rank == 0:
        metrics_logger.summary(confusion_count=confusion_count)
